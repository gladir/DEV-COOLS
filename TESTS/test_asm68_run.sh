...
#
# Ce script compile ASM68.PAS si necessaire, assemble les fichiers de test
# dans SAMPLES/ASM68/, puis execute les S-Records generes dans l'emulateur
# m68k_emu.py pour verifier que le code machine fonctionne correctement.
#
# =============================================================================
...
EMU="$REPO_DIR/TESTS/m68k_emu.py"
...
labels.asm macro.asm \
test_arith.asm test_encode.asm test_listing.asm test_logic.asm \
test_macro.asm test_shift.asm test_special.asm test_srecord.asm \
test_transfer.asm test_twopass.asm"

# Fichiers assemblage seul (testent des fonctionnalites de l'assembleur,
# pas concus pour l'execution)
...
for f in $EXEC_FILES; do
  ...
  # Executer dans l'emulateur
  EMU_OUT=$(python3 "$EMU" "$S68_PATH" --max-cycles 200000 2>&1)
  EMU_EXIT=$?
