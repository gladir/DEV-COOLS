; REXX86 v1.0 - SAMPLES/REXX/signal.rex
.MODEL SMALL
.STACK 1024
...
; SIGNAL ON HALT NAME ON_HALT
        MOV     WORD PTR [_RX_TRAP_HALT], OFFSET _RXP_ON_HALT
; SAY
        LEA     AX, _RXK_2
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL OFF HALT
        MOV     WORD PTR [_RX_TRAP_HALT], 0
; SAY
        LEA     AX, _RXK_3
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL ON NOVALUE NAME ON_NOVALUE
        MOV     WORD PTR [_RX_TRAP_NOVALUE], OFFSET _RXP_ON_NOVALUE
; SAY
        LEA     AX, _RXK_4
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL OFF NOVALUE
        MOV     WORD PTR [_RX_TRAP_NOVALUE], 0
; SAY
        LEA     AX, _RXK_5
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL ON ERROR NAME ON_ERROR
        MOV     WORD PTR [_RX_TRAP_ERROR], OFFSET _RXP_ON_ERROR
; SAY
        LEA     AX, _RXK_6
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL OFF ERROR
        MOV     WORD PTR [_RX_TRAP_ERROR], 0
; SAY
        LEA     AX, _RXK_7
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL ON SYNTAX NAME ON_SYNTAX
        MOV     WORD PTR [_RX_TRAP_SYNTAX], OFFSET _RXP_ON_SYNTAX
; SAY
        LEA     AX, _RXK_8
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
; SIGNAL OFF SYNTAX
        MOV     WORD PTR [_RX_TRAP_SYNTAX], 0
; SAY
        LEA     AX, _RXK_9
        MOV   SI, AX
        CALL   _RXRT_PRINTLN
...
_RX_HEAPTOP DW 0
_RXK_1 DB '=== SIGNAL ON ===',0
_RXK_2 DB 'SIGNAL ON HALT active',0
_RXK_3 DB 'SIGNAL OFF HALT fait',0
_RXK_4 DB 'SIGNAL ON NOVALUE active',0
_RXK_5 DB 'SIGNAL OFF NOVALUE fait',0
_RXK_6 DB 'SIGNAL ON ERROR active',0
_RXK_7 DB 'SIGNAL OFF ERROR fait',0
_RXK_8 DB 'SIGNAL ON SYNTAX active',0
_RXK_9 DB 'SIGNAL OFF SYNTAX fait',0
_RXK_10 DB '=== SIGNAL label ===',0
_RXK_11 DB 'Ceci est saute',0
_RXK_12 DB 'Saut effectue avec succes',0
_RXK_13 DB '=== SIGNAL VALUE ===',0
_RXK_14 DB 'Ceci est saute aussi',0
_RXK_15 DB 'SIGNAL VALUE effectue',0
_RXK_16 DB 'Fin du programme signal.rex',0
_RXK_17 DB 'Condition HALT interceptee',0
_RXK_18 DB 'Condition NOVALUE interceptee',0
_RXK_19 DB 'Condition ERROR interceptee',0
_RXK_20 DB 'Condition SYNTAX interceptee',0
_DATA ENDS
