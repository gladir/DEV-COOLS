...
; Genere automatiquement a partir de : SAMPLES/TURBOPASCAL/fileio.pas
...
; --- Variables File I/O (lecture/ecriture fichier texte) ---
FILE_NAMEBUF DB 260 DUP(0)
FILE_RDBUF   DB 256 DUP(0)
FILE_BUFPOS  DD 0
FILE_BUFEND  DD 0
FILE_EOFFLG  DD 0
FILE_BYTESRD DD 0
...
PARAM_COUNT  DD 0
PARAM_ARGC   DD 0
PARAM_PTRS   DD 64 DUP(0)
PARAM_BUF    DB 1024 DUP(0)
...
_TPK_1  DB 'test_output.txt',0
_TPK_2  DB 'Ligne 1: Hello from TP2JS',0
_TPK_3  DB 'Ligne 2: Deuxieme ligne',0
_TPK_4  DB 'Ligne 3: Nombre = ',0
_TPK_5  DB 'Fichier ecrit: test_output.txt',0
_TPK_6  DB 'Lu ligne ',0
_TPK_7  DB ': ',0
_TPK_8  DB 'Total lignes lues: ',0
_TPK_9  DB 'fichier_inexistant.xyz',0
_TPK_10  DB 'Erreur: fichier inexistant non trouve (attendu)',0
_TPK_11  DB 'Fichier inexistant ouvert (inattendu)',0
_TPK_12  DB 'Done.',0
_TPV_F  DD 0
_TPV_S  DB 256 DUP(0)
_TPV_I  DD 0
...
; writeln
        ...
        MOV DWORD PTR [FILE_WRHND],EAX
        LEA EAX,[_TPK_2]
        MOV ESI,EAX
        CALL _TPRT_FPRINTSTR
        LEA ESI,[CRLF]
        CALL _TPRT_FPRINTSTR
; writeln
        LEA EAX,[_TPV_F]
...
; readln
        LEA EAX,[_TPV_F]
        PUSH EAX
        CALL _TPRT_READLN_FILE
        ADD ESP,4
        LEA ESI,[INBUF]
        LEA EAX,[_TPV_S]
        MOV EDI,EAX
        CALL _TPRT_STRASSIGN
...
_TPRT_RESET:
        ...
        PUSH 1
        PUSH 80000000h
        PUSH OFFSET FILE_NAMEBUF
        CALL CreateFileA
        MOV EDI,DWORD PTR [EBP+8]
        MOV DWORD PTR [EDI],EAX
        MOV DWORD PTR [FILE_BUFPOS],0
        MOV DWORD PTR [FILE_BUFEND],0
        MOV DWORD PTR [FILE_EOFFLG],0
        POPAD
        POP EBP
        RET
...
_TPRT_EOF_FILE:
        ...
        CMP EAX,-1
        JE _TPRT_EF_YES
        MOV EAX,DWORD PTR [FILE_BUFPOS]
        CMP EAX,DWORD PTR [FILE_BUFEND]
        JB _TPRT_EF_NO
        CMP DWORD PTR [FILE_EOFFLG],1
        JE _TPRT_EF_YES
        PUSH 0
        PUSH OFFSET FILE_BYTESRD
        PUSH 256
        PUSH OFFSET FILE_RDBUF
        PUSH DWORD PTR [EBX]
        CALL ReadFile
        MOV EAX,DWORD PTR [FILE_BYTESRD]
        TEST EAX,EAX
        JZ _TPRT_EF_SET
        MOV DWORD PTR [FILE_BUFEND],EAX
        MOV DWORD PTR [FILE_BUFPOS],0
_TPRT_EF_NO:
        XOR EAX,EAX
        POP EDI
...
_TPRT_RF_GC:
        LEA ESI,[FILE_RDBUF]
        ADD ESI,EAX
        INC DWORD PTR [FILE_BUFPOS]
        MOVZX EAX,BYTE PTR [ESI]
        CMP AL,10
        JE _TPRT_RF_DN
        CMP AL,13
        JE _TPRT_RF_NX
        MOV BYTE PTR [INBUF+ECX],AL
        INC ECX
        CMP ECX,254
        JAE _TPRT_RF_DN
        JMP _TPRT_RF_NX
...
_TPRT_PC_SP:
        ...
        JMP _TPRT_PC_SK
_TPRT_PC_FN:
        MOV DWORD PTR [PARAM_ARGC],ECX
        DEC ECX
        CMP ECX,0
        JGE _TPRT_PC_OK
        XOR ECX,ECX
_TPRT_PC_OK:
        MOV DWORD PTR [PARAM_COUNT],ECX
        MOV DWORD PTR [PARAM_PARSED],1
        POPAD
_TPRT_PC_DN:
        RET
...
_TPRT_PARAMCOUNT:
        CALL _TPRT_PARSECMD
        MOV EAX,DWORD PTR [PARAM_COUNT]
        RET

_TPRT_PARAMSTR:
        CALL _TPRT_PARSECMD
        MOV ECX,DWORD PTR [ESP+4]
        CMP ECX,DWORD PTR [PARAM_ARGC]
        JGE _TPRT_PS_EM
        LEA EAX,[PARAM_PTRS]
        SHL ECX,2
        ADD EAX,ECX
