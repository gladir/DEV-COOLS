; Genere par PROLOG86 v1.0 - SAMPLES/PROLOG/test_io.pl
; Date: 2026-03-06
...
; --- Regle greet/0 clause 0 (1 but(s), 0 var(s)) ---
_PLP_greet_0_0:
        PUSH   BP
        MOV   BP, SP
; --- corps de la regle ---
...
; --- Regle greet_ln/0 clause 0 (1 but(s), 0 var(s)) ---
_PLP_greet_ln_0_0:
        PUSH   BP
        MOV   BP, SP
; --- corps de la regle ---
...
; --- Regle newline/0 clause 0 (1 but(s), 0 var(s)) ---
_PLP_newline_0_0:
        PUSH   BP
        MOV   BP, SP
; --- corps de la regle ---
...
; --- Regle show/1 clause 0 (1 but(s), 1 var(s)) ---
_PLP_show_1_0:
        PUSH   BP
        MOV   BP, SP
...
; --- Regle canon/1 clause 0 (1 but(s), 1 var(s)) ---
_PLP_canon_1_0:
        PUSH   BP
        MOV   BP, SP
; allocate 1 variable(s) locale(s)
...
; but 1/1: write_canonical/1
; write_canonical/1 inline (same as write)
        MOV   AX, WORD PTR [BP-2]
        CALL    _PLRT_DEREF
        CALL    _PLRT_WRITE_TERM
; write_canonical/1 done
        CLC
        MOV   SP, BP
        POP   BP
        RET
...
; --- Regle show_num/0 clause 0 (1 but(s), 0 var(s)) ---
_PLP_show_num_0_0:
        PUSH   BP
        MOV   BP, SP
; --- corps de la regle ---
...
; --- Regle indent/0 clause 0 (2 but(s), 0 var(s)) ---
_PLP_indent_0_0:
        PUSH   BP
        MOV   BP, SP
; --- corps de la regle ---
...
; --- Regle put_a/0 clause 0 (1 but(s), 0 var(s)) ---
_PLP_put_a_0_0:
        PUSH   BP
        MOV   BP, SP
; --- corps de la regle ---
...
; --- Regle code_of_a/1 clause 0 (1 but(s), 1 var(s)) ---
_PLP_code_of_a_1_0:
        PUSH   BP
        MOV   BP, SP
; allocate 1 variable(s) locale(s)
...
_PLP_code_of_a_1_0_F:
        STC
        MOV   SP, BP
        POP   BP
        RET

; --- Regle len_hello/1 clause 0 (1 but(s), 1 var(s)) ---
_PLP_len_hello_1_0:
        PUSH   BP
        MOV   BP, SP
; allocate 1 variable(s) locale(s)
...
_PLP_len_hello_1_0_F:
        STC
        MOV   SP, BP
        POP   BP
        RET

; --- Regle chars_of/2 clause 0 (1 but(s), 2 var(s)) ---
_PLP_chars_of_2_0:
        PUSH   BP
        MOV   BP, SP
...
; but 1/1: atom_chars/2
; atom_chars/2 inline
        MOV   AX, WORD PTR [BP-2]
        CALL    _PLRT_DEREF
        CALL    _PLRT_ATOM_CHARS
        MOV   BX, AX
...
; --- Regle codes_of/2 clause 0 (1 but(s), 2 var(s)) ---
_PLP_codes_of_2_0:
        PUSH   BP
        MOV   BP, SP
; allocate 2 variable(s) locale(s)
...
; but 1/1: atom_codes/2
; atom_codes/2 inline (same as atom_chars for codes)
        MOV   AX, WORD PTR [BP-2]
        CALL    _PLRT_DEREF
        CALL    _PLRT_ATOM_CHARS
        MOV   BX, AX
...
; --- Regle num_chars/2 clause 0 (1 but(s), 2 var(s)) ---
_PLP_num_chars_2_0:
        PUSH   BP
        MOV   BP, SP
; allocate 2 variable(s) locale(s)
...
; but 1/1: number_chars/2
; number_chars/2 inline
        MOV   AX, WORD PTR [BP-2]
        CALL    _PLRT_DEREF
        CALL    _PLRT_NUMBER_CHARS
        MOV   BX, AX
        MOV   AX, WORD PTR [BP-4]
        CALL    _PLRT_BIND
; number_chars/2 done
...
_PLP_num_chars_2_0_F:
        STC
        MOV   SP, BP
        POP   BP
        RET

; --- Regle num_codes/2 clause 0 (1 but(s), 2 var(s)) ---
_PLP_num_codes_2_0:
        PUSH   BP
        MOV   BP, SP
; allocate 2 variable(s) locale(s)
...
; but 1/1: number_codes/2
; number_codes/2 inline (same as number_chars)
        MOV   AX, WORD PTR [BP-2]
        CALL    _PLRT_DEREF
        CALL    _PLRT_NUMBER_CHARS
        MOV   BX, AX
...
; === Predicat greet_ln/0 (1 clause(s)) ===

; === Predicat newline/0 (1 clause(s)) ===

; === Predicat show/1 (1 clause(s)) ===
_PLP_show_1:
        JMP   _PLP_show_1_0

; === Predicat canon/1 (1 clause(s)) ===
_PLP_canon_1:
        JMP   _PLP_canon_1_0

; === Predicat show_num/0 (1 clause(s)) ===

; === Predicat indent/0 (1 clause(s)) ===

; === Predicat put_a/0 (1 clause(s)) ===

; === Predicat code_of_a/1 (1 clause(s)) ===
_PLP_code_of_a_1:
        JMP   _PLP_code_of_a_1_0

; === Predicat len_hello/1 (1 clause(s)) ===
_PLP_len_hello_1:
        JMP   _PLP_len_hello_1_0

; === Predicat chars_of/2 (1 clause(s)) ===
_PLP_chars_of_2:
        JMP   _PLP_chars_of_2_0

; === Predicat codes_of/2 (1 clause(s)) ===
_PLP_codes_of_2:
        JMP   _PLP_codes_of_2_0

; === Predicat num_chars/2 (1 clause(s)) ===
_PLP_num_chars_2:
        JMP   _PLP_num_chars_2_0

; === Predicat num_codes/2 (1 clause(s)) ===
_PLP_num_codes_2:
