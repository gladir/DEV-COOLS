...
; Source : SAMPLES\VAXPASCAL\test_string.pas
; ========================================
...
; assign s1
        LEA   AX, _VXPK_3
        MOV   WORD PTR [_VXP_S1], AX
; assign s2
        LEA   AX, _VXPK_4
        MOV   WORD PTR [_VXP_S2], AX
; assign s3
        MOV   AX, WORD PTR [_VXP_S1]
        PUSH   AX
        LEA   AX, _VXPK_5
        MOV   BX, AX
        POP   AX
...
; || concatenation
        ...
        MOV   DI, BX
        CALL   _VXPRT_CONCAT
        MOV   WORD PTR [_VXP_S3], AX
; writeln
        LEA   AX, _VXPK_7
        MOV   SI, AX
        CALL   _VXPRT_PRINTSTR
        MOV   AX, WORD PTR [_VXP_S3]
...
; assign len
        ...
        MOV   SI, AX
        CALL   _VXPRT_STRLEN
        MOV   WORD PTR [_VXP_LEN], AX
; writeln
        LEA   AX, _VXPK_8
        MOV   SI, AX
        CALL   _VXPRT_PRINTSTR
        MOV   AX, WORD PTR [_VXP_LEN]
        CALL   _VXPRT_NUMTOSTR
        MOV   SI, OFFSET _VXPRT_NUMBUF
        CALL   _VXPRT_PRINTSTR
        MOV   SI, OFFSET _VXPRT_CRLF
...
_VXPK_6  DB  '!',0
_VXPK_7  DB  's3 = ',0
_VXPK_8  DB  'length(s3) = ',0
_VXPK_9  DB  'substr(s3,1,5) = ',0
_VXPK_10  DB  'World',0
_VXPK_11  DB  'index(s3, World) = ',0
_VXPK_12  DB  'uppercase(s1) = ',0
_VXPK_13  DB  'lowercase(s2) = ',0
_VXPK_14  DB  'trim(s1) = ',0
_VXPK_15  DB  'pad(s1, 10) = ',0
_VXPK_16  DB  ' ',0
_DATA   ENDS
