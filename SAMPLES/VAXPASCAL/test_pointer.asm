...
; Source : SAMPLES/VAXPASCAL/test_pointer.pas
...
; deref p^
        ...
        CALL   _VXPRT_NEWLINE
; dispose(p)
        MOV   EAX, DWORD PTR [_VXP_P]
        CALL   _VXPRT_DISPOSE
; new(n1) size=8
        MOV   EAX, 8
        CALL   _VXPRT_NEW
        MOV   DWORD PTR [_VXP_N1], EAX
; new(n2) size=8
        MOV   EAX, 8
        CALL   _VXPRT_NEW
        MOV   DWORD PTR [_VXP_N2], EAX
; writeln
        LEA   EAX, _VXPK_4
        MOV   ESI, EAX
        CALL   _VXPRT_PRINTSTR
; deref n1^
        MOV   EBX, DWORD PTR [_VXP_N1]
        MOV   EAX, DWORD PTR [EBX]
        CALL   _VXPRT_PRINTINT
        CALL   _VXPRT_NEWLINE
; writeln
...
; deref n1^
        ...
        MOV   EAX, DWORD PTR [EBX]
        XOR   EAX, EAX
        MOV   DWORD PTR [_VXP_ADDR], EAX
; writeln
        LEA   EAX, _VXPK_6
        MOV   ESI, EAX
        CALL   _VXPRT_PRINTSTR
        MOV   EAX, DWORD PTR [_VXP_ADDR]
        CALL   _VXPRT_PRINTINT
        CALL   _VXPRT_NEWLINE
; dispose(n2)
        MOV   EAX, DWORD PTR [_VXP_N2]
        CALL   _VXPRT_DISPOSE
; dispose(n1)
        MOV   EAX, DWORD PTR [_VXP_N1]
        CALL   _VXPRT_DISPOSE
