...
; Source : SAMPLES\VAXPASCAL\test_set.pas
; ========================================
...
; set constructor
        ...
        MOV   AX, OFFSET _VXPRT_SETBUF
        MOV   SI, AX
        MOV   DI, OFFSET _VXP_S2
        CALL   _VXPRT_SET_COPY
; assign s3
        MOV   AX, OFFSET _VXP_S1
        PUSH   AX
        MOV   AX, OFFSET _VXP_S2
        MOV   BX, AX
        POP   AX
; set union
        MOV   SI, AX
        MOV   DI, BX
        CALL   _VXPRT_SET_UNION
        MOV   SI, AX
        MOV   DI, OFFSET _VXP_S3
        CALL   _VXPRT_SET_COPY
; writeln
        LEA   AX, _VXPK_3
        MOV   SI, AX
        CALL   _VXPRT_PRINTSTR
        MOV   AX, OFFSET _VXP_S3
        MOV   SI, AX
...
_VXP_S3  DB  32 DUP(0)
_VXP_TOTAL  DW  0
_VXPK_3  DB  'Card(s3) = ',0
_VXPK_4  DB  'Card(s1*s2) = ',0
_VXPK_5  DB  'Card(s1-s2) = ',0
_VXPK_8  DB  'Red est dans s1',0
_VXPK_11  DB  'Yellow est dans s1',0
_VXPK_12  DB  'Yellow n''est PAS dans s1',0
_VXPK_15  DB  's1 = s2',0
_VXPK_16  DB  's1 <> s2',0
_VXPK_19  DB  '[Green] <= s1',0
_DATA   ENDS
