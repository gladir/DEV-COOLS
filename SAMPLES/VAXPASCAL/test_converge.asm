...
; Source : SAMPLES/VAXPASCAL/test_converge.pas
...
; exception: sauvegarder handler parent
        ...
        MOV   DWORD PTR [_VXPRT_EXCHDL], OFFSET _VXPL_3
...
; exception: sauvegarder handler parent
        ...
        PUSH   DWORD PTR [_VXPRT_EXCSP]
        PUSH   DWORD PTR [_VXPRT_EXCBP]
        MOV   DWORD PTR [_VXPRT_EXCHDL], OFFSET _VXPL_9
        MOV   DWORD PTR [_VXPRT_EXCSP], ESP
        MOV   DWORD PTR [_VXPRT_EXCBP], EBP
; assign count
        MOV   EAX, DWORD PTR [_VXP_COUNT]
        PUSH   EAX
        MOV   EAX, 1
        MOV   EBX, EAX
        POP   EAX
        ADD   EAX, EBX
        MOV   DWORD PTR [_VXP_COUNT], EAX
; writeln
        LEA   EAX, _VXPK_11
        MOV   ESI, EAX
...
; assign sum
        XOR   EAX, EAX
        MOV   DWORD PTR [_VXP_SUM], EAX
; for
        MOV   EAX, 1
        MOV   DWORD PTR [_VXP_I], EAX
        MOV   EAX, DWORD PTR [EBP+0]
        MOV   DWORD PTR [_VXPL_15], EAX
_VXPL_16:
        MOV   EAX, DWORD PTR [_VXP_I]
        CMP   EAX, DWORD PTR [_VXPL_15]
        JG   _VXPL_17
; exception: sauvegarder handler parent
        PUSH   DWORD PTR [_VXPRT_EXCHDL]
        PUSH   DWORD PTR [_VXPRT_EXCSP]
        PUSH   DWORD PTR [_VXPRT_EXCBP]
        MOV   DWORD PTR [_VXPRT_EXCHDL], OFFSET _VXPL_19
        MOV   DWORD PTR [_VXPRT_EXCSP], ESP
        MOV   DWORD PTR [_VXPRT_EXCBP], EBP
; assign arr[i]
        MOV   EAX, DWORD PTR [_VXP_I]
        ...
        IMUL   EBX
        POP   EBX
        ADD   EBX, OFFSET _VXP_ARR
        MOV   DWORD PTR [EBX], EAX
; assign sum
        MOV   EAX, DWORD PTR [_VXP_SUM]
        PUSH   EAX
; index arr[i]
        MOV   EAX, DWORD PTR [_VXP_I]
        SUB   EAX, 1
...
; assign p.x
        MOV   EAX, 10
        MOV   DWORD PTR [_VXPR_P], EAX
; assign p.y
        MOV   EAX, 20
        MOV   EBX, OFFSET _VXPR_P
        ADD   EBX, 4
        MOV   DWORD PTR [EBX], EAX
; writeln
        LEA   EAX, _VXPK_23
...
; assign colors
; set constructor
        LEA   EDI, _VXPRT_STMP1
        XOR   EAX, EAX
        MOV   ECX, 8
        REP    STOSD
        XOR   EAX, EAX
        LEA   EDI, _VXPRT_STMP1
        BTS   DWORD PTR [EDI], EAX
        MOV   EAX, 2
        LEA   EDI, _VXPRT_STMP1
        BTS   DWORD PTR [EDI], EAX
        LEA   EAX, _VXPRT_STMP1
        MOV   ESI, EAX
        MOV   EDI, OFFSET _VXP_COLORS
        CALL   _VXPRT_SET_COPY
; if
        MOV   EAX, OFFSET _VXP_COLORS
        MOV   EBX, EAX
        MOV   EAX, 1
; element IN set
        MOV   ESI, EBX
...
; for
        ...
        MOV   DWORD PTR [_VXP_I], EAX
        MOV   EAX, 3
        MOV   DWORD PTR [_VXPL_37], EAX
_VXPL_38:
        MOV   EAX, DWORD PTR [_VXP_I]
        CMP   EAX, DWORD PTR [_VXPL_37]
        JG   _VXPL_39
; call tick
        CALL   _VXPF_TICK
_VXPL_40:
...
; new(ptr) size=4
        MOV   EAX, 4
        CALL   _VXPRT_NEW
        MOV   DWORD PTR [_VXP_PTR], EAX
; assign ptr^
        MOV   EAX, 999
        MOV   EBX, DWORD PTR [_VXP_PTR]
        MOV   DWORD PTR [EBX], EAX
; writeln
        LEA   EAX, _VXPK_41
        MOV   ESI, EAX
...
_VXPL_43:
        ...
        POP   EAX
        AND   EAX, EBX
        MOV   BYTE PTR [_VXP_OK], AL
; writeln
        LEA   EAX, _VXPK_44
        MOV   ESI, EAX
        CALL   _VXPRT_PRINTSTR
        MOVZX   EAX, BYTE PTR [_VXP_OK]
        CALL   _VXPRT_PRINTBOOL
        CALL   _VXPRT_NEWLINE
; exception: sauvegarder handler parent
        PUSH   DWORD PTR [_VXPRT_EXCHDL]
        PUSH   DWORD PTR [_VXPRT_EXCSP]
        PUSH   DWORD PTR [_VXPRT_EXCBP]
        MOV   DWORD PTR [_VXPRT_EXCHDL], OFFSET _VXPL_45
        MOV   DWORD PTR [_VXPRT_EXCSP], ESP
        MOV   DWORD PTR [_VXPRT_EXCBP], EBP
; signal(test_condition)
        MOV   DWORD PTR [_VXPRT_EXCCODE], 0
        MOV   ESP, DWORD PTR [_VXPRT_EXCSP]
        MOV   EBP, DWORD PTR [_VXPRT_EXCBP]
        JMP   DWORD PTR [_VXPRT_EXCHDL]
; exception: pas d erreur, restaurer handler parent
...
; exception: sauvegarder handler parent
        ...
        PUSH   DWORD PTR [_VXPRT_EXCSP]
        PUSH   DWORD PTR [_VXPRT_EXCBP]
        MOV   DWORD PTR [_VXPRT_EXCHDL], OFFSET _VXPL_53
        MOV   DWORD PTR [_VXPRT_EXCSP], ESP
        MOV   DWORD PTR [_VXPRT_EXCBP], EBP
; write
        MOV   EAX, DWORD PTR [_VXP_I]
...
_VXPK_1  DB  'Test Convergence',0
_VXP_ARR  DD  0
_VXPR_P  DB  8 DUP(0)
_VXP_COLORS  DB  32 DUP(0)
_VXP_I  DD  0
_VXP_SUM  DD  0
_VXP_S  DB  84 DUP(0)
_VXP_OK  DB  0
...
_VXP_COUNT  DD  0
_VXPK_11  DB  'tick #',0
_VXPK_14  DB  'Version ',0
_VXPL_15  DD  0
_VXPK_21  DB  'Sum of squares 1..',0
_VXPK_22  DB  ' = ',0
_VXPK_23  DB  'Point : (',0
_VXPK_24  DB  ', ',0
_VXPK_25  DB  ')',0
_VXPK_28  DB  'Green present',0
_VXPK_29  DB  'Green absent',0
_VXPK_30  DB  'Card = ',0
_VXPK_31  DB  'Hello',0
_VXPK_32  DB  ' ',0
_VXPK_33  DB  'World',0
_VXPK_34  DB  's = ',0
_VXPK_35  DB  'length = ',0
_VXPK_36  DB  'fib(10) = ',0
_VXPL_37  DD  0
_VXPK_41  DB  'ptr^ = ',0
_VXPK_44  DB  'ok = ',0
_VXPK_48  DB  'test_condition intercepte',0
_VXPK_49  DB  'Erreur inattendue',0
_VXPK_55  DB  ' ',0
_VXPK_58  DB  ' ',0
_VXPK_60  DB  'Tous les tests convergent. OK.',0
_DATA   ENDS
        END     _VXPF_MAIN
